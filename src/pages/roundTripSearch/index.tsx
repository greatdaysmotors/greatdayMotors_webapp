...
import { art_seats, availTrips_returnTrips_type, storeState, TripData_v1 } from "../../types/Trip";
...
const RoundTripSearch = () => {
  ...
  const trip_data = use_round_trip((state: storeState) => state.trip_data);
       ...
       // GETTING ROUNDTRIP FORM DATA FROM STORE
    const trip_details = use_round_trip((state: storeState) => state.round_trip_post_data);
    const set_trip_details = use_round_trip((state: storeState) => state.set_round_trip_post_data);
  ...
  //>>>>>>>>>>>>> GETTING "TERMINAL FROM DB TO SET TERMINAL ID TO TERMINAL" NAME STARTS HERE>>>>>>>>>>>>>>>>>>>>>>>>>>
  useEffect(() => {
  ...
  useEffect(() => {
    if(trip_data.departure_terminal !== ''){
      if (data) {
        const terminal_list = data.result;
  ...
  const showModal = (id:string, cost:number) => {
    set_departure_trip_id(id)
    set_departure_trip_cost(cost)
    setOpen(true);
  };
  const show_return_trip_modal = (id:string, cost:number) => {
      ...
      case 1:
        return (
          <Two_way_personal_info
            ...
            numberOfBeneficiaries={trip_data.number_of_adults > 1 ? trip_data.number_of_adults - 1 : 0}
  ...
  const fetch_available_trips = async (token: string | null) => {
    const response = await fetch(`${BASE_URL}/v1/passenger/round-trip/${trip_data.departure_terminal}/${trip_data.arrival_terminal}/${trip_data.departure_date}/${trip_data.arrival_date}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const { 
    ...
    isLoading: available_trip_loading } = useQuery({
    queryKey: ["fetch_available_trips", userToken],
    queryFn: () => fetch_available_trips(userToken),
    enabled: !!userToken,
  });

  useEffect(() => {
    if (available_trip_data) {
      if (available_trip_data.availableTrips.length !== 0 && available_trip_data.returnTrips.length !== 0) {
...
const run_seat_clicked= (seat_number:number)=>{
  if (all_selected_array.includes(seat_number)) {
    ...
    const edited_array = all_selected_array.filter((item) => item !== seat_number);
...
const run_seat_clicked2= (seat_number:number)=>{
  if (all_selected_array2.includes(seat_number)) {
    // Use filter to create a new array without the seat_number
    const edited_array = all_selected_array2.filter((item) => item !== seat_number);
...
<div className="w-full flex justify-center mt-8">
        <Spin />
      </div>:available_trip_data?
<>

{tripData &&
                      tripData.availableTrips.map((item:availTrips_returnTrips_type, index:number) => (
                        <div
                        key={index.toString()}
                        >
                        <SelectTrip
                          ...
                          price={item.tripCost}
                          onSelectSeat={() => showModal(item._id,item.tripCost)}
                        />
...
closable={false}
title={null}
onOk={ ()=> set_show_return_trips(value => !value)}
  ...
  <div className="md:py-16">
    ...
    <div className="flex flex-col items-center justify-between mt-[2.4rem]">
      <div className="flex w-full items-center justify-between px-10">
        ...
        <div
          className={`relative ${
            item.seatsAndStatus[0].seatStatus === "available" ? "cursor-pointer" : "pointer-events-none"
          } `}
          // onClick={() =>
          //   item.seatsAndStatus[0].seatStatus === "available" && handleSeatClick(item.seatsAndStatus[0].seatNumber)
          // }
        >
          <FaUser size={35} color="#e6e6e6" />
      ...
      <div className="flex flex-wrap gap-10 justify-center">
        {item.seatsAndStatus.slice(1).map((seat:art_seats,index) => (
    seat_clicked === true && all_selected_array.includes(seat.seatNumber)?
    <div
    key={index.toString()}
    className={`relative mt-[2.4rem] ${
      seat.seatStatus === "available" ? "cursor-pointer": "pointer-events-none"
    } `}
    onClick={()=>{
   if(trip_data.number_of_adults){

    if (all_selected_array.length < trip_data.number_of_adults || all_selected_array.includes(seat.seatNumber)){
      run_seat_clicked(seat.seatNumber)
    }
   }
...
>
  <FaUser size={35} color={getSeatColor(seat.seatStatus)} />
  <span className="text-white text-[1.4rem] absolute top-[80%] left-1/2 transform -translate-x-1/2 -translate-y-1/2">
    {seat.seatNumber}
  </span>
</div>
    ...
    <Button
      ...
      ${
        all_selected_array.length === trip_data.number_of_adults
          ...
          `}
      disabled={all_selected_array.length < trip_data.number_of_adults}
    >
      Continue
    </Button>
  </div>,
...
{tripData &&
              tripData.returnTrips.map((item:availTrips_returnTrips_type, index:number) => (
                <div
                key={index.toString()}
                >
                <SelectTrip
                  ...
                  departureTime={item.departureDateTime}
                  price={item.tripCost}
                  onSelectSeat={()=>show_return_trip_modal(item._id,item.tripCost)}
                />
...
<div className="flex flex-wrap gap-10 justify-center">
{item.seatsAndStatus.slice(1).map((seat:art_seats,index) => (
seat_clicked2 === true && all_selected_array2.includes(seat.seatNumber)?
<div
key={index.toString()}
className={`relative mt-[2.4rem] ${
seat.seatStatus === "available" ? "cursor-pointer": "pointer-events-none"
...
if (all_selected_array2.length < trip_data.number_of_adults || all_selected_array2.includes(seat.seatNumber)){
run_seat_clicked2(seat.seatNumber)
}
}
...
all_selected_array2.length === trip_data.number_of_adults
  ...
  `}
disabled={all_selected_array2.length < trip_data.number_of_adults}
>
Continue
</Button>
</div>,
